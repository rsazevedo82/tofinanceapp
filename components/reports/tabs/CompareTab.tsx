            ...
            {(['income', 'expense'] as const).map(type => {
              ...
              return (
                <div key={type} className="p-3 rounded-xl bg-white" style={{ border: '1px solid #D1D5DB' }}>
                  <p className="text-[10px] uppercase tracking-widest font-semibold mb-2" style={{ color: type === 'income' ? '#2DD4BF' : '#FF7F50' }}>
                    {type === 'income' ? 'Receitas' : 'Despesas'}
                  </p>
                  <p className="text-base font-black text-[#0F172A]">{fmtCur(curV)}</p>
                  <p className="text-xs mt-1 text-[#6B7280]">Anterior: {fmtCur(prevV)}</p>
                  {prevV > 0 ? (
