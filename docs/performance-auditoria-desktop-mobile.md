### 1) Bundle muito alto na página de relatórios (`/relatorios`) [Criticidade: Crítica]
...
### 2) Renderização excessiva no cliente em páginas principais [Criticidade: Crítica]
...
### 3) Estratégia de polling/refetch agressiva no dashboard [Criticidade: Alta]
...
### 4) Endpoint de convite de casal com `listUsers()` completo [Criticidade: Alta]
...
### 5) Endpoint de relatórios com alto custo computacional no servidor [Criticidade: Alta]
...
### 6) Falta de code-splitting para modais/forms pesados [Criticidade: Alta]
...
### 7) Recharts carregado integralmente mesmo sem aba ativa [Criticidade: Alta]
...
### 8) Invalidação ampla de queries após mutações [Criticidade: Média]
...
### 9) Cálculos repetidos em render no cliente (listas grandes) [Criticidade: Média]
...
### 10) Configuração PWA agressiva com cache de navegação/API [Criticidade: Média]
...
### 11) Tabelas e listas sem virtualização para crescimento de volume [Criticidade: Média]
